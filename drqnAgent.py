...
pre_train_episode = 5 # 학습 시작 전에 몇번의 무작위 episode를 할 것인가.
            ...
            if i >= pre_train_episode:
                ...
                if total_steps % (update_freq) == 0:
                    ...
                    Q1 = sess.run(mainQN.predict, feed_dict={ \
                        mainQN.scalarInput: np.vstack(trainBatch[:, 3] / 6.0), \
                        mainQN.trainLength: trace_length, mainQN.state_in: state_train, mainQN.batch_size: batch_size})
                    # 타겟 신경망에서 Q 값들을 얻는다.
                    Q2 = sess.run(targetQN.Qout, feed_dict={ \
                        targetQN.scalarInput: np.vstack(trainBatch[:, 3] / 6.0), \
                    ...
                    sess.run(mainQN.updateModel, \
                             feed_dict={mainQN.scalarInput: np.vstack(trainBatch[:, 0] / 6.0),
